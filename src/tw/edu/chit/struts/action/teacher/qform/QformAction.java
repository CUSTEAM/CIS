...
public class QformAction extends BaseLookupDispatchAction{
	...
	public ActionForward unspecified(ActionMapping mapping, ActionForm form,
		...
		if(request.getParameter("doid")!=null){			
			...
			if(manager.ezGetInt("SELECT COUNT(*)FROM AQ_anser WHERE idno='"+c.getMember().getIdno()+"'")<3){
				session.setAttribute("tqst", true);//鎖定
		...
		//初入
		//UserCredential c = (UserCredential) session.getAttribute("Credential");		
		session.setAttribute("dept", manager.ezGetString("SELECT c.idno2 FROM CodeEmpl c, empl e WHERE e.unit=c.idno AND e.idno='"+c.getMember().getIdno()+"'"));
		session.setAttribute("depts", manager.ezGetBy("SELECT * FROM AQ_unit WHERE id='Q'"));
		session.setAttribute("myduty", manager.ezGetString("SELECT Director FROM empl WHERE idno='"+c.getMember().getIdno()+"'"));		
	...
	public ActionForward Continue(ActionMapping mapping, ActionForm form,
		...
		//request.setAttribute("other", manager.ezGetBy("SELECT * FROM AQ_unit WHERE id='R'"));//所有系所
		request.setAttribute("qs", manager.ezGetBy("SELECT * FROM AQ_question WHERE uid='Q'"));
		ActionMessages msg = new ActionMessages();
		msg.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("Course.messageN1","問卷已準備，請作答"));
		saveMessages(request, msg);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		if(ans.trim().equals("")){
			//System.out.println("DELETE FROM AQ_anser WHERE Oid="+ans_oid);
