...
public class StudentOnlineAddDelCourseNewAction extends BaseLookupDispatchAction {
	...
	protected List<OnlineAddRemoveCourseUpAction.SeldDataInfo> getSeldDataListByIndex(
		...
		for (OnlineAddRemoveCourseUpAction.SeldDataInfo sdi : seldDataList) {
			if (Toolket.isValueInCookie(sdi.getSeldOid().toString(), index)) {
				...
				synchronized (sdi) {
					stuSelect = cm.countSelect((sdi.getDtimeOid()).toString());
...
//			}
	...
	protected List<Map> doSomethingForOpencs(Student student,
		...
		for (Map map : contents) {
			seldCount = cm.findSeldCountByDtimeOid((Integer) map.get("DtimeOID"));
...
//					(String) map.get("begin"), (String) map.get("end")));
			...
			if (!ArrayUtils.contains(IConstants.COURSE_ARMY, (String) map
					.get("cscode"))
					&& "2".equals((String) map.get("opt"))
					&& StringUtils.indexOf((String) map.get("chi_name"), "體育") == StringUtils.INDEX_NOT_FOUND) {
				if (StringUtils.isBlank(((String) map.get("eng_name")))) {
					cis = cm.getCourseIntroByDtimeOid((Integer) map.get("DtimeOID"),
					...
					if (!cis.isEmpty()) {
						...
						if (StringUtils.isBlank(ci.getEngName())) {
							map.put("canChoose", "1");
							map.put("errorMessage", "科目英文名稱未輸入");
							// ret.add(map);
							continue;
						}
					}
			...
			if (isHist) {
				...
				if (0 == selLimit) {
					...
					boolean bFlag = stuSel.intValue() >= selLimit;
					if (bFlag) {
						map.put("canChoose", "1");
	...
	private Map<String, Object> processSeldCourse(List<Map> seldList,
		...
		boolean bFlag = false, bFlag1 = false, isLiteracyClass = false, isNeedCheck = true;
	...
	protected List<Map> processRoom(List<Map> contents) {
		...
		try {
			for (Map c : contents) {
				dcs = cm.hqlGetBy("FROM DtimeClass WHERE dtimeOid = "
						+ (Integer) c.get("DtimeOID"));
				if (!dcs.isEmpty()) {
					dc = dcs.get(0);
					nabbr = (Nabbr) cm
							.hqlGetBy(
									"FROM Nabbr WHERE roomId = '"
											+ dc.getPlace() + "'").get(0);
					building = nabbr.getBuilding();
					code5 = Toolket.findBuildingBy(building);
					c.put("name2", code5 != null ? code5.getName() : "");
					c.put("time2", getClassTimeInfo(dc.getWeek(),
							dc.getBegin(), dc.getEnd()));
					ret.add(c);
				} else
					ret.add(c);
			}
	...
	@SuppressWarnings("unchecked")
	protected List<Map> processRoomBak(List<Map> contents) {
		CourseManager cm = (CourseManager) getBean(COURSE_MANAGER_BEAN_NAME);
		List<Map> ret = new LinkedList<Map>();
