...
public class PhaseTwoAddDelCourseNewAction extends StudentOnlineAddDelCourseNewAction {
...
//						.getSchoolNo());
	...
	public ActionForward chooseAdd(ActionMapping mapping, ActionForm form,
		...
		UserCredential credential = getUserCredential(session);
		// 3部制不允許跨選,不允許跨系
		// 98.03.31改為第二,三階段同系同年級(或低年級)可跨部制修課
		...
		List<Map> contents = processRoom(cm.findOpencsByCondition4Phase2And3(
				clazz.getCampusNo(), clazz.getSchoolNo(), clazz.getDeptNo(),
				grade, clazz.getClassNo(), term));

		String schoolType = cm.findSchoolTypeByClassNo(credential.getStudent()
				.getDepartClass());
		session.setAttribute("opencsInfo", doSomethingForOpencs(credential
				.getStudent(), contents, cm.findMinCountBySchoolNo(clazz
				.getSchoolNo()), schoolType, (Boolean) session
