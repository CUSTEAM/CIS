...
public class SimpleUploaderServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String Uid=null;
		Uid=c.getMember().getAccount();//取得使用者帳號
		...
		if(enabled) {
			...
			try {
				...
				if(extIsAllowed(typeStr,ext)) {
					...
					if(pathToSave.getPath().lastIndexOf("\\")>0){//windows
						path=pathToSave.getPath().substring(0, pathToSave.getPath().lastIndexOf("\\"))+"\\";
					}else{//linux
						path=pathToSave.getPath().substring(0, pathToSave.getPath().lastIndexOf("/"))+"/";
					}
				}
		...
		FtpClient ftp=new FtpClient(manager.myPortfolioFtp(), "CIS", "cust!@#", "", "");
		
		ftp.connect();
		//String ServerDir=manager.FTPServerPath()+"s"+Uid+"/";
		
		String ServerDir=manager.FTPServerPath();
		//String WWWFilePath=""+manager.myPortfolioFtpClient()+"s"+Uid+"/"+fileName;
		String WWWFilePath=""+manager.myPortfolioFtpClient()+fileName;
		try{
		...
		String list[]=ftp.listNames(ServerDir);
		...
		out.println("window.parent.OnUploadCompleted("+retVal+",'"+WWWFilePath+"','"+newName+"','"+errorMessage+"');");
		out.println("</script>");
		out.flush();
		out.close();
