...
public class CardLoginAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, 
		...
		try {
			...
			UserCredential user = mm.createUserCredential(aForm.getString("username"), 
					cm.ezGetString("SELECT password FROM wwpass WHERE username='"+aForm.getString("username")+"'"));
			if(user==null){
	...
	private void SaveMoreInfomation(HttpServletRequest request, HttpSession session, UserCredential user) throws ParseException{
		...
		//Date coanswStart=sf.parse(manager.ezGetString("SELECT Value FROM Parameter WHERE Name='CoanswStart'"));//教學評量開始日
		//Date coanswEnd=sf.parse(manager.ezGetString("SELECT Value FROM Parameter WHERE Name='CoanswEnd'"));//教學評量結束日
		...
		if(user.getMember().getPriority().equals("A")){//若是同仁
			...
			if(session.getAttribute("myOnlineWork")==null){//線上申請
			...
			if(user.getMember().getPriority().equals("C")){				
				...
				msg.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("Course.messageN1", "通識"+map.get("opt3")+"學分<br>查看系所 " +
				"<a href='#' onClick=\"window.open('/CIS/getStdCredit.do?ClassNo="+user.getStudent().getDepartClass()+"', 'preview', 'height=480,width=640, toolbar=0, location=0, menubar=0', 'false');\">學分規劃</a></div><font size=-1><b>註<sup>1</sup></b> " +
				"學分數採及格成績即時統計,後續須經過畢業資格審查, <br><b>註<sup>2</sup></b> 學分規劃如變動仍以權責單位公佈為準</font>"));
			}
			//manager.setCoansFoRm(session, user, coanswStart, coanswEnd);
		}
