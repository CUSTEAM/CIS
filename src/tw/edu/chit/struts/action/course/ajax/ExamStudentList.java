 ...
 *
 */
public class ExamStudentList extends HttpServlet{
			...
			*/
			...
			for(int i=0; i<dtimeList.size(); i++){
				...
				if(!cscode.equals("50000")&& !cscode.equals("T0001")&& !cscode.equals("T0002") ){
					...
					cell.add(doEncodeKaiu("學期 "+title+"考 考場記載表\n\n", 12));
					cell.setColspan(6);
					...
					for(int j=0; j<students.size()+(students.size()%5); j++){
						//System.out.println(((Map)students.get(j)).get("student_name"));
						try{
							cells = new PdfPCell(doEncode(((Map)students.get(j)).get("student_no").toString()+
									((Map)students.get(j)).get("student_name").toString(), 9));
						}catch(Exception e){
					...
					if(students.size()<60){
						for(int j=students.size(); j<60; j++){
							cells = new PdfPCell(doEncode(" ", 9));
							cells.setBorder(0);
							tables.addCell(cells);
						}
