...
public class StudentScoreHistoryAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (scoreHistList.isEmpty()) {
			...
			boolean isOver10Char = false, isOver15Char = false, isSameDoubleMajorDeptCode = false;
								...
								if (1 == term) {
									...
									if (duplicatedCourseName
										...
										if (isNotExemptAndMend) {
											...
											//if()
											try{
												Toolket.setCellValue(sheet,32+ (isMaster ? -2: 0)+ (nextPage ? nextPageRowIndex: 0),k * 6 + 5,(stavgs.isEmpty() ? "": df.format(stavgs.get(0).getScore() + 0.001D)));
						...
						buffer
								.append(
										gm.getSchoolYear()
												...
												+ gm.getSchoolTerm()
												+ " "
												+ Toolket.getStatus(gm
														.getOccurStatus(),
														false)
												+ " "
												+ Toolket.getStatusCause(gm
														.getOccurCause())
												+ " "
