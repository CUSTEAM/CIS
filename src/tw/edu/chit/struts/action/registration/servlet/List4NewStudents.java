...
public class List4NewStudents extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		header.setRight(HSSFHeader.font("Stencil-Normal", "Normal")
				+ HSSFHeader.fontSize((short) 10) + "第" + HSSFFooter.page()
				+ "頁, 共" + HSSFFooter.numPages() + "頁");
		...
		try {
			...
			if (students.size() > 0) {// 有人畢業的話
				...
				String year = (((Map) students.get(0)).get("depart_class")
						.toString().substring(2, 3));
				if (year.equals("2")) {
					year = "二";
				}
				if (year.equals("4")) {
				...
				if (year.equals("G")) {
					header.setCenter(HSSFHeader.font("標楷體", "Normal")
							+ HSSFHeader.fontSize((short) 18) + schoolName
							+ " " + schoolYear + "學年度第" + schoolTerm + title);
					header.setLeft(manager.getClassInfo(((Map) students.get(0))
							.get("depart_class").toString(), 's'));
				} else {
					header.setCenter(HSSFHeader.font("標楷體", "Normal")
							+ HSSFHeader.fontSize((short) 18) + schoolName
							+ year + "年制 " + schoolYear + "學年度第" + schoolTerm
							+ title);
					header.setLeft(manager.getClassInfo(((Map) students.get(0))
							.get("depart_class").toString(), 's'));
				}
				...
				c = new HSSFRichTextString("這個查詢結果沒有新生...");
			...
			for (int i = 0; i < students.size(); i++) {
				...
				// 科系
				c = new HSSFRichTextString(((Map) students.get(i)).get("fname")
						.toString());
				cell = row.createCell((short) 1);
				cell.setCellValue(c);
				cell.setCellStyle(contentStyle);
				...
				// 名子
				c = new HSSFRichTextString(((Map) students.get(i)).get(
				...
				// 身份證
				c = new HSSFRichTextString(((Map) students.get(i)).get("idno")
						.toString());
				cell = row.createCell((short) 5);
				cell.setCellValue(c);
				cell.setCellStyle(contentStyle);
				...
				// 出生日期
				c = new HSSFRichTextString(manager.convertDate(d2.format(d1
						.parse(((Map) students.get(i)).get("birthday")
								.toString()))));
				cell = row.createCell((short) 6);
				cell.setCellValue(c);
				cell.setCellStyle(contentStyle);
				...
				if (((Map) students.get(i)).get("schl_name") != null
						&& !((Map) students.get(i)).get("schl_name").equals("")) {
					c = new HSSFRichTextString(((Map) students.get(i)).get(
							"schl_name").toString());
				} else {
					c = new HSSFRichTextString(
							manager
									.ezGetString("SELECT school_name FROM schl WHERE schl_code='"
											+ ((Map) students.get(i))
													.get("schl_code") + "'"));
				}
				...
				if (((Map) students.get(i)).get("grad_dept") != null
						&& !((Map) students.get(i)).get("grad_dept").equals("")) {
					c = new HSSFRichTextString(((Map) students.get(i)).get(
							"grad_dept").toString());
				} else {
					c = new HSSFRichTextString("");
			...
			sheet.setColumnWidth((short) 1, (short) 3800);
