...
public class ScoreHistAvgEditAction extends BaseLookupDispatchAction {
	...
	public ActionForward unspecified(ActionMapping mapping,
			...
			if (!"".equals(studentNo) || !"".equals(stdepartClass)) {
				...
				request.setAttribute("ScoreHistAvgEdit", map);
			}
	...
	public ActionForward cancel(ActionMapping mapping,
			...
			session.removeAttribute("ScoreHistAvgEdit");
	...
	public ActionForward save(ActionMapping mapping,
		...
		throw new Exception("'mode' property of ScoreHistAvgEditForm is illegal.");
	}
	...
	private ActionForward create(ActionMapping mapping,
		...
		if (!messages.isEmpty()) {
			...
			session.setAttribute("ScoreHistAvgEdit", form.getMap());
			return mapping.findForward("Main");
		} else {	
			try {
				...
				messages = sm.createScoreHistAvg(form.getMap(), classInCharge);
				if(! messages.isEmpty()){
					saveErrors(request, messages);
					session.setAttribute("ScoreHistAvgEdit", form.getMap());
					return mapping.findForward("Main");
	...
	private ActionForward modify(ActionMapping mapping,
		...
		HttpSession session = request.getSession(false);
		Stavg score = (Stavg)session.getAttribute("ScoreAvgInEdit");
		...
		if (!messages.isEmpty()) {
			...
			try {
				ScoreManager sm = (ScoreManager)getBean("scoreManager");
				sm.modifyScoreHistAvg(form.getMap(), score);
	...
	private ActionMessages validateInputForCreate(DynaActionForm form, ResourceBundle bundle) {
		...
		ScoreManager sm = (ScoreManager)getBean("scoreManager");
		sm.validateScoreHistAvgCreate(form.getMap(), errors, bundle);
		return errors;
	}
	
	private ActionMessages validateInputForModify(DynaActionForm form, Stavg score, ResourceBundle bundle) {
		// score:old data , form: new fill data, form->studentNo,cscode,school_year,school_term can't modify
		ActionMessages errors = new ActionMessages();
		validateFieldFormat(form, errors, bundle);
		ScoreManager sm = (ScoreManager)getBean("scoreManager");
		sm.validateScoreHistAvgModify(form.getMap(), score, errors, bundle);
		return errors;
	}
