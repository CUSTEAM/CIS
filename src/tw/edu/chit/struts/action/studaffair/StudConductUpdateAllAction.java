...
public class StudConductUpdateAllAction extends BaseLookupDispatchAction {
	...
	public ActionForward unspecified(ActionMapping mapping, ActionForm form,
		...
		setContentPage(session, "studaffair/StudConductUpdateAll.jsp");
	...
	public ActionForward cancel(ActionMapping mapping,
		...
		HttpSession session = request.getSession(false);
		setContentPage(request.getSession(false), "studaffair/StudConductUpdateAll.jsp");
		return mapping.findForward("Main");
	}
	...
	public ActionForward update(ActionMapping mapping, ActionForm aform,
		...
		session.setAttribute("ConductUpdateInit", initMap);
		...
		if("All".equals(campus)||"All".equals(school)){
			...
			try {
				msgs = sm.conductUpdate(clazzFilter);
		...
		msgs.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(
				"MessageN1", clazzFilter + " 操行成績更新完成!!!"));
		saveMessages(request, msgs);
		session.setMaxInactiveInterval(sessionInterval);	//recovery for session timeout
