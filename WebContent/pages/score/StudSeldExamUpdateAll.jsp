...
<form action="/CIS/Score/StudSeldExamUpdateAll.do" method="post" name="caForm">
...
<script>generateTableBanner('<div class="gray_15"><B><bean:message key="SAFTitle.StudSeldExamUpdateAll" bundle="SAF"/></B></div>');</script>	  
	<tr>
		<td align="left"> 
<!-- End Content Page Table Header -->
	...
	<table cellspacing="5" class="empty-border" width="100%">
		<tr>
			<td colspan="2"><br/>
			本程式功能為更新所選擇部制學生扣考節數資料<br/>
			<font color="red" style="size:20px;">執行時間較久，請耐心等候...</font>
			<br/><br/></td>
		</tr>
	    <tr>
	        <td align="center" valign="middle">請選擇班級 ：
        	<c:set var="campusSel" value="${SeldExamUpdateInit.campus}"/>
	  		<c:set var="schoolSel" value="${SeldExamUpdateInit.school}"/>
	  		<c:set var="deptSel"   value="${SeldExamUpdateInit.dept}"/>
	  		<c:set var="classSel"  value="${SeldExamUpdateInit.departClass}"/>
	...
	<script>
	generateTableBanner('<INPUT typeforms[0]" name="method" value="<bean:message key='OK'/>" id="ok" onClick="if(reconfirm()) document.forms[0].submit();">&nbsp;&nbsp;'+
	'<INPUT type="buforms[0]me="method" value="<bean:message key='Cancel'/>" id="cancel" onClick="chgopmode(0);document.forms[0].submit();">');
	</script>
</table>
...
function clearBar() {
	...
	function reconfirm() {
		...
		if(confirm("確定要重新計算學生選課曠缺紀錄嗎？")) {
			chgopmode(1);
			button_ok.disabled = true;
			button_cancel.disabled = true;
			// alert("urlprefix:" + urlprefix);
			pollurl = urlprefix + "/AjaxGetRunningStatus?process=updateSeldExamAll";
