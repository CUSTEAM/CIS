...
<html:form action="/Teacher/TeachSyllabus" method="post" onsubmit="init('系統處理中...')">
	<script>
		generateTableBanner('<div class="gray_15"><B><bean:message key="teacher.syllabus.banner" bundle="TCH"/> - <font color="red" size="+1">${schoolYear}</font>&nbsp;學年度 第<font color="red" size="+1">${TechSyllabusForm.map.term}</font>學期 </B></div>');
	</script>
    ...
    <tr>
    	<td align="center">
    		<font color="red" size="+1">輸入課程綱要之起迄時間：${begin} ~ ${end}</font>
    	</td>    	
    </tr>
    -->
    <tr>
	...
	<tr>
	 			...
	 			<tr>
	 				<td align="center">
	 					<div id="term1" style="display:inline;">
      					<display:table name="${teacherDtime}" export="false" id="row" pagesize="10000" sort="list" excludedParams="*" class="list">
        					...
        					<display:column title="下載" sortable="false" class="center" href="/CIS/Course/SyllabusPrint.do" paramId="Oid" paramProperty="oid">
        						<img src="images/tag_pink.gif" border="0" alt="下載大綱" />
        					</display:column> 					
      					</display:table>
      					</div>
      					<!--
   	...
   	<script>
   		generateTableBanner('<INPUT type="submit" name="method" value="<bean:message key='teacher.introductionSearch.btn.search' bundle="TCH" />" class="CourseButton">&nbsp;' + 
   							'<INPUT type="submit" name="method" value="<bean:message key='teacher.syllabus.btn.add' bundle="TCH" />" onclick="return courseCheck();" class="CourseButton">&nbsp;');
   	</script>
	<!--/c:if-->
</html:form>
</table>
