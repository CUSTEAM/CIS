	...
	<tr>
		...
		<table class="hairLineTable" align="left">
			<tr>				
				...
				<select name="sid">
					<option <c:if test="${EmplStaticWorkManageForm.map.sid==''}">selected</c:if> value="">全部班別</option>
					<c:forEach items="${allShift}" var="as">
					<option <c:if test="${EmplStaticWorkManageForm.map.sid==as.id}">selected</c:if> value="${as.id}">${as.name}</option>				
					</c:forEach>
				</select>
		...
		<table class="hairLineTable" align="left">
			<tr>
				...
				<td class="hairLineTd">
				<select name="sunit">
					<option <c:if test="${EmplStaticWorkManageForm.map.sunit==''}">selected</c:if> value="">全部單位</option>
					<c:forEach items="${allUnit}" var="c">
					<option <c:if test="${EmplStaticWorkManageForm.map.sunit==c.idno}">selected</c:if> value="${c.idno}">${c.name}</option>
					</c:forEach>
				</select>
				</td>
				</td>
	...
	<tr>		
		...
		<table class="hairLineTable" align="left">
			<tr>
				...
				name="sidno" id="fsidno" size="12" value="${EmplStaticWorkManageForm.map.sidno}" onFocus="chInput(this.id)"
				...
				onMouseOver="showHelpMessage('姓名輸入這裡, 若您貼上文字, 請按一下鍵盤右側的方向鍵, 自動完成身分證號', 'inline', this.id)" 
 				onMouseOut="showHelpMessage('', 'none', this.id)"
				type="text" name="fscname" id="fscname" size="10" value="${EmplStaticWorkManageForm.map.fscname}"
