...
**
**/
function createSelItems(inputObj, field_name, jsonObj, func){
	clearNames();
	...
	for(var i=0; i<size; i++){
		...
		//window.alert(json);
		var nextNode = jsonObj[i][field_name];
		row = document.createElement("tr");
		cell = document.createElement("td");
		cell.onmouseout = function(){this.className='fullColorTrF';};
		cell.onmouseover = function(){this.className='fullColorTr';};
		...
		cell.setAttribute("id", jsonObj[i]["idno"]);
		if(func){
...
function populateName(inputObj, cell){
	...
	inputObj.value = cell.firstChild.nodeValue;
	clearNames();
}
...
function getEmpl(mfObj, isSelItem){
	autoOrigtext = mfObj.value;
	var url = "/CIS/AjaxGlobal";
	var jsonQuery = '{"method":"getAgent", "sname":"' + mfObj.value + '"}';
...
**
*/
function setAutoInput_agent(){
// parameters: inputObj, field_name, orig_text, isSelItem, func
	if(handleState()){
		var field_name = "cname";
		var orig_text = autoOrigtext;
		var isSelItem = true;
		var func = chgAgentId;
		var inputObj = document.getElementById("fscname");
			...
			if(jsonObj){
				...
				if(orig_text == new_input){
					if(!isSelItem){
						...
						createSelItems(inputObj, field_name, jsonObj, func);
					}else{
						clearNames();
					}
				}else{
