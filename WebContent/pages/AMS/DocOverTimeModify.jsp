...
<input name="docType" type="hidden" value="${DocOverTimeModifyInit.docType}"/>
...
<script>generateTableBanner('加班管理->修改');</script>	  
<!-- End Content Page Table Header -->
	<tr><td><table>
	<c:choose>
	<c:when test="${DocOverTimeModifyForm != null}">
	<c:set var="formback" value="${DocOverTimeModifyForm}"></c:set>
	...
	<c:when test="${DocOverTimeInEdit != null}">
	<c:set var="formback" value="${DocOverTimeInEdit}"></c:set>
	<input name="oid" type="hidden" value="${formback.oid}"/>
	<tr>
		<td width="75" class="title">加班原因</td>
		<td class="input"><input type="text" name="reason" class="mustInput" value="${formback.reason}"></td>
	...
	<tr>
		...
		<td class="input">
		<select name="totalDay">
			<c:if test="${formback.totalHour == 4}">
			<option value="0.5" selected="selected">0.5</option>
			<option value="1">1</option>
			</c:if>
			<c:if test="${formback.totalDay == 1}">
	...
	<tr>
		...
		<select name="endMinute" id="startMinute">
			<option value="0">0</option>
			<option value="30">30</option>
		</select> 分
		</td>
	...
	<tr>
		...
		<table class="descript" width="100%">
			...
			<td>
加班單請於加班日前或當日送至人事室核備，逾期不予受理。Xxx</td>
			</tr>
			<tr>
