...
<input name="docType" type="hidden" value="${DocAskLeaveEditInit.docType}"/>
...
<!-- End Content Page Table Header -->
	<tr><td><table>
	<c:choose>
	<c:when test="${DocAskLeaveEditForm != null}">
	<c:set var="formback" value="${DocAskLeaveEditForm}"></c:set>
	<tr>
		...
		<td class="input">
			<table class="hairLineTable">
				<tr>
					...
					姓名 <input type="text" autocomplete="off" name="fscname" id="fscname" size="10"  class="mustInput" value="${formback.fscname}"
								onMouseOver="showHelpMessage('姓名輸入這裡!', 'inline', this.id)" 
				   				onMouseOut="showHelpMessage('', 'none', this.id)"
				   				onClick = "this.value=''; document.getElementById('ids').value=''; document.getElementById('agent').value='';";
	...
	<tr>
		<td width="75" class="title">請假事由</td>
		<td class="input"><input type="text" maxlength="13" name="reason" class="mustInput">(最多可輸入13個中文字)${DocAskLeaveEditForm}</td>
	</tr>
	<tr>
		<td width="75" class="title">假別</td>
		<td class="input">
	...
	<tr>
		...
		<td class="input">
			<table class="hairLineTable">
				<tr>
					<td class="input" align="left" class="mustInput">
					姓名 <input type="text" autocomplete="off" name="fscname" id="fscname" size="10"  class="mustInput" value=""
								onMouseOver="showHelpMessage('姓名輸入這裡!', 'inline', this.id)" 
				   				onMouseOut="showHelpMessage('', 'none', this.id)"
				   				onClick = "this.value=''; document.getElementById('ids').value=''; document.getElementById('agent').value='';";
							 	onkeyup="if(this.value.length>0)getEmpl(this, true);"/>
	...
	<tr>
		...
		<table class="descript">
			...
			<td>
【請假期限】：教師請假須填寫假單一至三聯，職員工只須填二、三聯。假單第一聯專供教師因涉及授課之異動與調整時，須將此聯交由教務處存查外，第二聯由各單位（或本人）留存，第三聯送交人事室登記。所有假單或出勤證明在經批准後，需於請假日之隔日起7日內（含例假日）送交人事室才算完成請假手續，逾時恕不受理。</td>
			</tr>
			<tr>
			<td width="20" valign="top">2.</td>
			<td>
【主管請假程序】：各單位一級主管請假，需依行政程序經校長簽准。</td>
			</tr>
			<tr>
			<td width="20" valign="top">3.</td>
			<td>
【教職員工請假程序】：除公差、公假、產假外之各種假別，請假二日（含）內，職員工由單位一級主管核准；教師有課者加會簽教務處，兼導師者加會學務處，經系主任、院長簽准。請假日數在二天（不含）以上者，需經副校長核准；五天（含）以上者，需再經校長核准。</td>
			</tr>
			<tr>
			<td width="20" valign="top">4.</td>
			<td>
【公差與公假】：職員工公差一日（含）內由單位一級主管核准；公差達二日（含）以上、公假及須申請差旅費者，均需再經校長核准。教師公差、公假需經校長核准。</td>
			</tr>
			<tr>
			<td width="20" valign="top">5.</td>
			<td>
【產假】：各種產假於送校長批示前，請先會人事室。</td>
			</tr>
			<tr>
			<td width="20" valign="top">6.</td>
			<td>
【證明文件】：請長病假、婚喪產假與公假者，須隨附證明文件之正本。</td>
			</tr>
			<tr>
...
【年假與補休】：同組職員工不可同時請年假或補休。年假最少以半天為單位；補休最少以一小時為單位。在非規定上班時間內，因奉學校或單位主管之命，加班或出差達四小時以上者（時數不得跨日累計），才得申請補休四小時，並須以加班當日之刷卡紀錄或校外出差之公文假單為憑。</td>
			...
			<td>
【新竹分部教職員工請假程序】：無論主管或一般教職員工，假單除須經分部副校長簽核外，其他均按一般請假規定。</td>
			</tr>
			<tr>
