...
<input name="docType" type="hidden" value="${DocRevokeModifyInit.docType}"/>
...
<script>generateTableBanner('銷假單管理->修改');</script>	  
<!-- End Content Page Table Header -->
	...
	<c:set var="askleave" value="${DocRevokeInEdit.revokedDoc}"></c:set>
	<tr>
	...
	<c:choose>
	<c:when test="${DocRevokeModifyForm != null}">
	<c:set var="formback" value="${DocRevokeModifyForm}"></c:set>
	...
	<c:when test="${DocRevokeInEdit != null}">
	<c:set var="formback" value="${DocRevokeInEdit}"></c:set>
	<input name="oid" type="hidden" value="${formback.oid}"/>
	<tr>
		<td width="75" class="title">銷假原因</td>
		<td class="input"><input type="text" name="reason" class="mustInput" value="${formback.reason}"></td>
