...
<c:if test="${BPSPrintReport != null}">
<c:set var="reportPath" value="${BPSPrintReport.reportPath}" />
<c:set var="reportTitle" value="${BPSPrintReport.reportTitle}" />
...
<!--
	...
	function preview(){
		...
		for(i=0; i<mode.length; i++){
			if(mode[i].checked) modev=mode[i].value;
		}
		...
		if(isValidate) {
			...
			subwin = window.open("/CIS/StudAffair/BonusPenaltySum.do?method=PrintPreview" + str, "預覽列印",
...
<form action="/CIS/StudAffair/BonusPenaltySum.do" method="post" name="ceForm" onsubmit="init('報表產生中, 請稍後')">
<table width="100%" cellpadding="0" cellspacing="0">

<script>generateTableBanner('<div class="gray_15"><B><bean:message key="SAFTitle.BonusPenaltySum" bundle="SAF"/></B></div>');</script>	  
	<tr>
		<td>
<!-- End Content Page Table Header -->
	...
	<table width="100%" cellspacing="5" class="empty-border">
      ...
      <tr>
      <tr>
        <td  height="30" width="10%" align="left" valign="middle" bgcolor="#d8bfd8">班級：      
        <td  height="30" width="90%" align="left" valign="middle">
        	<c:set var="campusSel" value="${BPSumInit.campus}"/>
	  		<c:set var="schoolSel" value="${BPSumInit.school}"/>
	  		<c:set var="deptSel"   value="${BPSumInit.dept}"/>
	  		<c:set var="classSel"  value="${BPSumInit.departClass}"/>
			<%@ include file="/pages/studaffair/include/ClassSelectSAF.jsp" %>
        </td>
      </tr>
      <tr>
        <td height="30" width="10%" align="left" valign="middle" bgcolor="#d8bfd8">報教育部：      
      	<td width="90%">
      		<input type="radio" name="mode" value="0" checked>否
     		<input type="radio" name="mode" value="1">是
     	</td>
      </tr>
	</table>
		</td>		
