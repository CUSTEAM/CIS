...
<input name="oid" type="hidden" value="${StudPubDocInEdit.oid}"/>
...
<script>generateTableBanner('公假管理->修改');</script>	  
...
<!-- End Content Page Table Header -->
	<tr><td><table>
	<c:choose>
	<c:when test="${StudPubDocModifyForm != null}">
	<c:set var="formback" value="${StudPubDocModifyForm}"></c:set>
	<!-- 
	<tr>
	...
	-->
	<input type="hidden" name="department" value="${formback.department}">
	<tr>
		<td width="75" class="title">班級</td>
		<td class="input">${formback.deptClassName}</td>
	</tr>
	...
	<c:when test="${StudPubDocInEdit != null}">
	<c:set var="formback" value="${StudPubDocInEdit}"></c:set>
	<input type="hidden" name="department" value="${formback.department}">
	<tr>
		<td width="75" class="title">班級</td>
		<td class="input">${formback.deptClassName}</td>
	...
	<tr>
		<td class="title">開始日期、節次</td>
        <td class="hairlineTdF">
            <input type="text" name="startDate" size="10" maxlength="8" readonly="readonly" value="${formback.simpleStartDate}">
    			<img src="<%=basePath%>pages/images/cal.gif" name="calendar" width="20" height="20"
		      		align="top" style="cursor:hand" alt="點選此處選擇日期"
		  	  		onclick="javascript:if(!startDate.disabled)popCalFrame.fPopCalendar('startDate','startDate',event);">&nbsp;&nbsp;&nbsp;
		  	  		節次：
	...
	<tr>
        <td class="title">結束時間、節次</td>
        <td class="hairlineTdF">
            <input type="text" name="endDate" size="10" maxlength="8" readonly="readonly" value="${formback.simpleEndDate}">
    			<img src="<%=basePath%>pages/images/cal.gif" name="calendar" width="20" height="20"
    				align="top" style="cursor:hand" alt="點選此處選擇日期"
 		  	  		onclick="javascript:if(!endDate.disabled)popCalFrame.fPopCalendar('endDate','endDate',event);">&nbsp;&nbsp;&nbsp;
		  	  		節次：
	...
	<tr>
		...
		<tr><td>
			<DIV id="studentList">
			<c:forEach items="${formback.studDoc}" var="stdoc">
					<div  class="style2" id="div_${stdoc.studentNo}">
					<input type="hidden" name="studentNo" value="${stdoc.studentNo}">
					<table width="100%" id="tb_${stdoc.studentNo}">
					<tr><td>${stdoc.deptClassName}&nbsp;&nbsp;${stdoc.studentName}(${stdoc.studentNo})</td>
					<td><input type="button" align="right" class="buttons" value="刪除" onclick="delStud(${stdoc.studentNo});">
					</td></tr></table></div>
			</c:forEach>
			</DIV>
		</td></tr>
...
function addNewStudPeriod(studNo, depart){
	...
	var jsonQuery = '{"method":"chkStudentInClass", "studentNo":"' + studNo + '","clazz":"' + depart + '"}';
