...
<form action="/CIS/StudAffair/StudConductCreateAll.do" method="post" name="caForm">
...
<!-- End Content Page Table Header -->
	...
	<table cellspacing="5" class="empty-border" width="100%">
		<tr>
			<td><br/>
			注意！！！<br/>
			本程將重新建立所選擇部制學生操行成績，原系統中之資料將被刪除<br/>
			曠缺加減分將會被設定為+3,總分將會被設定為85分<br/> -->
			請於建立完畢後再執行更新操行成績步驟!<br/>
			<br/></td>
		</tr>
	    <tr>
	        <td align="center" valign="middle">部制 ：
        	<c:set var="campusSel" value="${ConductCreateInit.campus}"/>
	  		<c:set var="schoolSel" value="${ConductCreateInit.school}"/>
	  		<c:set var="deptSel"   value="${ConductCreateInit.dept}"/>
	  		<c:set var="classSel"  value="${ConductCreateInit.departClass}"/>
...
function clearBar() {
	...
	function reconfirm() {
		...
		if(confirm("確定要重新建立操行成績嗎？")) {
			chgopmode(1);
			button_ok.disabled = true;
			button_cancel.disabled = true;
			// alert("urlprefix:" + urlprefix);
			pollurl = urlprefix + "/AjaxGetRunningStatus?process=createJustAll";
