...
function preview(isPreview) {
	...
	var str = "&c=" + doc.campusInCharge2.value + "&s=" + doc.schoolInCharge2.value + "&d=" + doc.deptInCharge2.value +
		...
		"&rank=" + rankType + "&tn=" + doc.termNum.value +
		"&sst=" + scholarshipType;
	if (isPreview && bFlag) {	
		// alert(str);
		subwin = window.open("/CIS/StudAffair/ReportPrint.do?method=preview" + str, "預覽列印",
...
<html:form action="/StudAffair/ReportPrint" method="post" onsubmit="init('報表產生中, 請稍後')">
				...
				<tr>
					<td>
						<html:select property="printOpt" onchange="showHelp(this.value)">
							...
							<option value="ArmyList1" <c:if test="${ReportPringForm.map.opt == 'ArmyList1'}">selected</c:if>>儘後召集第二款申請處理名冊</option>
							<option value="ArmyList2" <c:if test="${ReportPringForm.map.opt == 'ArmyList2'}">selected</c:if>>儘後召集第二款延長修業申請處理名冊</option>
							<option value="ArmyList3" <c:if test="${ReportPringForm.map.opt == 'ArmyList3'}">selected</c:if>>儘後召集第二款原因消滅名冊</option>
							<option value="ArmyList4" <c:if test="${ReportPringForm.map.opt == 'ArmyList4'}">selected</c:if>>離校學生緩徵原因消滅名冊</option>
							<option value="ArmyList5" <c:if test="${ReportPringForm.map.opt == 'ArmyList5'}">selected</c:if>>申請緩徵學生名冊</option>
							<option value="ArmyList6" <c:if test="${ReportPringForm.map.opt == 'ArmyList6'}">selected</c:if>>延長修業年限學生名冊</option>
							<option value="ArmyList7" <c:if test="${ReportPringForm.map.opt == 'ArmyList7'}">selected</c:if>>僑生離校名冊</option>
						</html:select>
