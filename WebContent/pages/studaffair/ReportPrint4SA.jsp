...
function preview(isPreview) {
	...
	if (isPreview && bFlag) {	
		// alert(str);
		subwin = window.open("/CIS/StudAffair/ReportPrint4SA.do?method=preview" + str, "預覽列印",
...
<html:form action="/StudAffair/ReportPrint4SA" method="post" onsubmit="init('報表產生中, 請稍後')">
				...
				<tr>
					<td>
						<html:select property="printOpt" onchange="showHelp(this.value)">
							...
							<option value="StayTimeList" <c:if test="${ReportPringForm.map.opt == 'StayTimeList'}">selected</c:if>>導師留校時間列印</option>
						</html:select>
