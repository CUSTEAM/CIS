...
<c:if test="${JO95PrintReport != null}">
<c:set var="reportPath" value="${JO95PrintReport.reportPath}" />
<c:set var="reportTitle" value="${JO95PrintReport.reportTitle}" />
...
<!--
	...
	function preview(){
		...
		if(isValidate) {
			...
			subwin = window.open("/CIS/StudAffair/JustOver95.do?method=PrintPreview" + str, "預覽列印",
...
<form action="/CIS/StudAffair/JustOver95.do" method="post" name="ceForm" onsubmit="init('報表產生中, 請稍後')">
<table width="100%" cellpadding="0" cellspacing="0">

<script>generateTableBanner('<div class="gray_15"><B><bean:message key="SAFTitle.JustOver95" bundle="SAF"/></B></div>');</script>	  
	<tr>
		<td>
<!-- End Content Page Table Header -->
	...
	<table width="100%" cellspacing="5" class="empty-border">
      ...
      <tr>
      	<td colspan="2">
      		<font color="red">
      		<p> !!!  請注意：本程式不再計算曠缺課及獎懲的加減分，如要重新計算，可執行重新計算全校學生操行成績的程式，然後再來列印 !</p>
      		 </font>
      	</td>
      </tr>
      <tr>
        ...
        <td  height="30" width="92%" align="left" valign="middle">
        	<c:set var="campusSel" value="${JOver95Init.campus}"/>
	  		<c:set var="schoolSel" value="${JOver95Init.school}"/>
	  		<c:set var="deptSel"   value="${JOver95Init.dept}"/>
	  		<c:set var="classSel"  value="${JOver95Init.departClass}"/>
