...
<c:if test="${JLDPrintReport != null}">
<c:set var="reportPath" value="${JLDPrintReport.reportPath}" />
<c:set var="reportTitle" value="${JLDPrintReport.reportTitle}" />
...
<!--
	...
	function preview(){
		...
		if(isValidate) {
			...
			subwin = window.open("/CIS/StudAffair/JustLevelD.do?method=PrintPreview" + str, "預覽列印",
...
<form action="/CIS/StudAffair/JustLevelD.do" method="post" name="ceForm" onsubmit="init('報表產生中, 請稍後')">
<table width="100%" cellpadding="0" cellspacing="0">

<script>generateTableBanner('<div class="gray_15"><B><bean:message key="SAFTitle.JustLevelD" bundle="SAF"/></B></div>');</script>	  
	<tr>
		<td>
	...
	<table width="100%" cellspacing="5" class="empty-border">
      ...
      <tr>
        ...
        <td  height="30" width="92%" align="left" valign="middle">
        	<c:set var="campusSel" value="${JLevelDInit.campus}"/>
	  		<c:set var="schoolSel" value="${JLevelDInit.school}"/>
	  		<c:set var="deptSel"   value="${JLevelDInit.dept}"/>
	  		<c:set var="classSel"  value="${JLevelDInit.departClass}"/>
			<%@ include file="/pages/studaffair/include/ClassSelectSAF.jsp" %>
        </td>
      </tr>
      <tr>
        <td height="30" width="8%" align="left" valign="middle" bgcolor="#d8bfd8">等第：      
      	<td width="92%">
      		<input type="radio" name="level" value="4" checked>丁等
     		<input type="radio" name="level" value="5">定察
     	</td>
      </tr>
	</table>
		</td>		
