...
function preview(isPreview) {
	...
	if (isPreview && bFlag) {	
		// alert(str);
		subwin = window.open("/CIS/FEE/ReportPrint.do?method=preview" + str, "預覽列印",
...
<html:form action="/FEE/ReportPrint" method="post" onsubmit="init('報表產生中, 請稍後')">
	...
	<tr>
		<td>
			<table class="empty-border">
				...
				<tr>
					<td><span id="xxx" style="display:none;"><bean:message key="OpenCourse.label.classNumber" bundle="COU"/>:
				...
				<tr>
					<td>
						<html:select property="printOpt" onchange="showHelp(this.value)">
							...
							<option value="StmdFeePayList" <c:if test="${ReportPringForm.map.opt == 'StmdFeePayList'}">selected</c:if>>收費檔學生名冊</option>
							<option value="ClassFeePayList" <c:if test="${ReportPringForm.map.opt == 'ClassFeePayList'}">selected</c:if>>班級學雜費明細資料</option>
							<option value="File4Yun" <c:if test="${ReportPringForm.map.opt == 'File4Yun'}">selected</c:if>>製作財政部扣繳稅額資料</option>
