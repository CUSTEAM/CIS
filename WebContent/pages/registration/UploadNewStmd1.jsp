...
<html:form action="/Registration/UploadNewStmd1" method="post" enctype="multipart/form-data" onsubmit="init('報表產生中, 請稍後')">
<table width="100%" cellpadding="0" cellspacing="0" border="0">
<script>
	generateTableBanner('<table align="left"><tr><td align="left">&nbsp;&nbsp;<img src="images/16-cube-debug.png"></td><td> 上 傳 新 生 基 本 資 料 (已 編 班 編 學 號)</td></tr></table>');
</script>
	<tr>
		<td>
	...
	<tr>
		<td>
			...
			<display:table name="${newStmdList}" export="false" id="row" pagesize="10" sort="list" excludedParams="*" class="list">
				...
				<display:column title="入學年月" property="entrance" sortable="true" class="center" />
				<display:column title="畢業學校" property="schlName" sortable="true" class="center" />
				<display:column title="畢業學校代碼" property="schlCode" sortable="true" class="center" />
				<display:column title="畢業科系" property="gradDept" sortable="true" class="center" />
				<display:column title="畢業年度" property="gradyear" sortable="true" class="center" />
				<display:footer>
					<c:if test="${not empty newStmdList}">
					<tr>
						<td>共解析筆數: <fmt:formatNumber value="${sessionScope.newStmdCount}" /> 筆</td>
					</tr>
					</c:if>	
				</display:footer>
			</display:table>
