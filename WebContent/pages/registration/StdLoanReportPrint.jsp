...
function preview(isPreview) {
	...
	if (isPreview && bFlag) {	
		// alert(str);
		subwin = window.open("/CIS/Registration/LoanReport/StudentLoanReport.do?method=preview" + str, "預覽列印",
...
<html:form action="/Registration/LoanReport/StudentLoanReport" method="post" onsubmit="init('報表產生中, 請稍後')">
				...
				<tr>
					<td>
						<html:select property="printOpt" onchange="showHelp(this.value)">
							...
							<option value="StdLoanList" <c:if test="${ReportPringForm.map.opt == 'StdLoanList'}">selected</c:if>>學生就學貸款</option>
						</html:select>
