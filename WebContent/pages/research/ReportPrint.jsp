...
function preview(isPreview) {
	...
	if (isPreview && bFlag) {	
		// alert(str);
		subwin = window.open("/CIS/Research/ReportPrint.do?method=preview" + str, "預覽列印",
...
<html:form action="/Research/ReportPrint" method="post" onsubmit="init('報表產生中, 請稍後')">
				...
				<tr>
					<td>
						<html:select property="printOpt" onchange="showHelp(this.value)">
							...
							<option value="GstmdList" <c:if test="${ReportPringForm.map.opt == 'GstmdList'}">selected</c:if>>歷屆畢業生資料</option>
...
function showHelp(type) {
	...
	if(type == 'GstmdList') {
		...
		document.getElementById('helpMsg').innerHTML = ('&nbsp;直接選擇學年度即可。');
	}
