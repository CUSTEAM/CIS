...
Style[9]=["white","black","#00B333","#E8E8FF","","","","","","","","","","",150,"",2,2,10,10,51,0.5,75,"simple","gray"];
Text[12]=["中英文簡介說明", "點選此處了解此課程之簡介說明..."];
...
<html:form action="/Student/PhaseOneAddDelCourse" method="post" onsubmit="init('系統處理中...')">
	<script>generateTableBanner('<div class="gray_15"><B><bean:message key="student.phaseOneAddDelCourse.banner" bundle="STD"/></B></div>');</script>
		...
		<td>
			...
			<display:table name="${seldDataInfoOpt1}" export="false" id="row" pagesize="10000" sort="list" excludedParams="*" class="list">
		        ...
		        <display:column titleKey="Course.label.courseName" sortable="false" class="left">
		        	${row.csName}&nbsp;
		        	<html:link page="/Student/PhaseOneAddDelCourse.do?method=readCourseIntro" paramName="row" paramId="oid" paramProperty="dtimeOid" onmouseover="stm(Text[12], Style[9])" onmouseout="htm()">
						[<font color="blue">中英文簡介</font>]
					</html:link>&nbsp;
					<html:link page="/Student/PhaseOneAddDelCourse.do?method=readCourseSyllabus" paramName="row" paramId="oid" paramProperty="dtimeOid" onmouseover="stm(Text[0], Style[0])" onmouseout="htm()">
		        ...
		        <display:column titleKey="Course.label.classTime" property="time2" sortable="false" class="left" />
		    </display:table>
		</td>
	</tr>
	</c:if>
	...
	<tr>
		<td>
			...
			<display:table name="${seldDataInfoOpt2}" export="false" id="row" pagesize="10000" sort="list" excludedParams="*" class="list">
		        ...
		        <display:column titleKey="Course.label.opt" property="optName" sortable="false" class="center" />
		        <display:column title="型態" property="elearningName" sortable="false" class="center" />
	...
	<!-- 這段是"查詢非本班加退選結果"按鈕  + '<INPUT type="submit" name="method" value="<bean:message key='course.onlineAddRemoveCourse.deleteCourseResult' bundle="COU" />" class="CourseButton">&nbsp;' -->
   	<c:choose>
   		<c:when test="${not empty seldDataInfoOpt2}">
   			<script>
	 			...
	 			<tr>
	 				<td align="center">
      					<display:table name="${opencsList}" export="false" id="row" pagesize="10000" sort="list" excludedParams="*" class="list">
					        ...
					        <display:column titleKey="Course.label.classTime" property="time2" sortable="false" class="center" />
      					</display:table>
      				</td>
      			</tr>
    		</table>
