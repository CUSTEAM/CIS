...
function preview(isPreview) {
	...
	if (document.getElementsByName("amountDateType")[0].checked)
		amountDateType = document.getElementsByName("amountDateType")[0].value;
	else
		amountDateType = document.getElementsByName("amountDateType")[1].value;
	...
	var str = "&c=" + doc.campusInCharge2.value + "&s=" + doc.schoolInCharge2.value + "&d=" + doc.deptInCharge2.value +
		...
		"&printInterClass=" + innerClass + "&ft=" + failType + "&rt=" + reportType + "&adt=" + amountDateType +
		"&sd=" + doc.startDate.value + "&ed=" + doc.endDate.value;
	if (isPreview && bFlag) {	
		// alert(str);
		subwin = window.open("/CIS/DeptAssistant/ReportPrint.do?method=preview" + str, "預覽列印",
...
<html:form action="/DeptAssistant/ReportPrint" method="post" onsubmit="init('報表產生中, 請稍後')">
			...
			<table class="empty-border">
				<tr>
					<td>
	    				...
	    				<span id="amountDate" style="display:none;">
    						<input type="radio" id="amountDateType" name="amountDateType" checked value="y"/>已補助&nbsp;<input type="radio" id="amountDateType" name="amountDateType" value="n"/>未補助
    					&nbsp;&nbsp;</span>
    				</td>    				
				</tr>
				<tr>
				...
				<tr>
					<td>
						<html:select property="printOpt" onchange="showHelp(this.value)">
							...
							<option value="AbilityList" <c:if test="${ReportPringForm.map.opt == 'AbilityList'}">selected</c:if>>證照資料列印(舊功能)</option>
							<option value="StdSkillList" <c:if test="${ReportPringForm.map.opt == 'StdSkillList'}">selected</c:if>>報部證照資料列印(舊功能)</option>
							<option value="Rc1-7" <c:if test="${ReportPringForm.map.opt == 'Rc1-7'}">selected</c:if>>1-7學術活動列印</option>
							<option value="Rc1-8" <c:if test="${ReportPringForm.map.opt == 'Rc1-8'}">selected</c:if>>1-8承接政府部門計劃與產學案列印</option>
							<option value="Rc1-9" <c:if test="${ReportPringForm.map.opt == 'Rc1-9'}">selected</c:if>>1-9期刊論文發表列印</option>
							<option value="Rc1-10" <c:if test="${ReportPringForm.map.opt == 'Rc1-10'}">selected</c:if>>1-10研討會論文發表列印</option>
							<option value="Rc1-11" <c:if test="${ReportPringForm.map.opt == 'Rc1-11'}">selected</c:if>>1-11專書(篇章)列印</option>
							<option value="Rc1-12" <c:if test="${ReportPringForm.map.opt == 'Rc1-12'}">selected</c:if>>1-12專利列印</option>
							<option value="Rc1-13" <c:if test="${ReportPringForm.map.opt == 'Rc1-13'}">selected</c:if>>1-13獲獎與榮譽列印</option>
							<option value="DeptStdSkillList" <c:if test="${ReportPringForm.map.opt == 'DeptStdSkillList'}">selected</c:if>>系所學生證照列印(匯入資料驗證用)</option>
							<option value="DeptStdSkill4EnglishList" <c:if test="${ReportPringForm.map.opt == 'DeptStdSkill4EnglishList'}">selected</c:if>>系所學生英文證照列印</option>
							<option value="DeptGStdSkillList" <c:if test="${ReportPringForm.map.opt == 'DeptGStdSkillList'}">selected</c:if>>系所休退畢轉學生證照列印</option>
							...
							<option value="DeptStdSkillList-4" <c:if test="${ReportPringForm.map.opt == 'DeptStdSkillList-4'}">selected</c:if>>學生考取專業證照報名費補助總表</option>
							...
							<option value="DeptStdSkillList-6" <c:if test="${ReportPringForm.map.opt == 'DeptStdSkillList-6'}">selected</c:if>>學生通過技能檢定證照人數明細表</option>
...
function showHelp(type) {
	...
	if(type == 'Syllabus') {
		...
		document.getElementById('amountDate').style.display = 'none';
	...
	if (type == 'DeptStdSkillList' || type == 'DeptStdSkill4EnglishList') {
		document.getElementById('checkCredit').style.display = 'none';
		document.getElementById('schoolType').style.display = 'none';
		document.getElementById('alert').style.display = 'inline';
		document.getElementById('point').style.display = 'inline';
	...
	if (type == 'DeptStdSkillList-1' || type == 'DeptStdSkillList-3'
		|| type == 'DeptStdSkillList-5' || type == 'DeptStdSkillList-6') {
		document.getElementById('checkCredit').style.display = 'none';
		document.getElementById('schoolType').style.display = 'none';
		document.getElementById('alert').style.display = 'inline';
		document.getElementById('point').style.display = 'inline';
	...
	if (type == 'DeptStdSkillList-2' || type == 'DeptStdSkillList-4') {
		document.getElementById('checkCredit').style.display = 'none';
		document.getElementById('schoolType').style.display = 'none';
		document.getElementById('alert').style.display = 'inline';
		document.getElementById('point').style.display = 'inline';
		...
		document.getElementById('calendarYear').style.display = 'none';
		document.getElementById('licenseDateRange').style.display = 'inline';
		document.getElementById('amountDate').style.display = 'inline';
		document.getElementById('helpMsg').innerHTML = ('&nbsp;');	
	}
