// TJPzoom 3 configuration file * János Pál Tóth
// 2007.04.28
// Docs @ http://valid.tjp.hu/tjpzoom/ 
// News @ http://tjpzoom.blogspot.com/

// NATGEO
// no drop shadow (see TJPshadowthick)
// changed zoom window size
// thick yellow border

var TJPzoomwidth=132;              //zoom window size
var TJPzoomheight=185; 
var TJPzoomwindowlock=0;           //set to 1 to lock window size

var TJPzoomamount=1.2;
var TJPzoomamountmax=12;
var TJPzoomamountmin=1;

var TJPborderthick=15;              //border thickness, SET 0 to no borders
var TJPbordercolor='#f4df4e';      //border color

var TJPshadowthick=0;              //shadow image size/2, SET 0 to have no shadows (saves cpu)
var TJPshadow='/CIS/pages/images/halobevel/';       // <>: n, ne, e, se, s, sw, w, nw - TJPshadow+'nw.png'

// TJPzoom 3 configuration file * János Pál Tóth
// Docs @ http://valid.tjp.hu/tjpzoom/ 
// News @ http://tjpzoom.blogspot.com/
