...
function init(message) {
    var loadingMessage;
  	...
  	document.getElementById('loadMsg').style.width=99999;
	document.getElementById('loadMsg').style.height=99999;
	...
	document.getElementById('loadMsgSub').innerHTML=message;
	document.getElementById('loadIco').style.left=Math.floor(Math.random()*(window.screen.availWidth-300));
	document.getElementById('loadIco').style.top=Math.floor(Math.random()*(window.screen.availHeight-300));if (navigator.appName.indexOf("Microsoft")>=0){
    	document.getElementById('icoimage').innerHTML='<img src="/CIS/pages/images/indicator/indicator'+Math.floor(Math.random()*5)+'.gif"/>';
    }
